import type { DataTableResponsiveColumnVisibility, DataTableState } from '@/components/ui/custom/data-table/interface';
import { valueUpdater } from '@/components/ui/table/utils';
import type { PaginatedDataCollection } from '@/spatie/types';
import type {
    ColumnPinningState,
    PaginationState,
    RowSelectionState,
...
type PaginatedCollection<TData> = PaginatedDataCollection<number, TData>;

export type UseDataTableOptions<TData> = {
    data: MaybeRefOrGetter<PaginatedCollection<TData> | TData[]>;
    responsiveColumnVisibility?: DataTableResponsiveColumnVisibility;
} & Pick<
...
export function useDataTable<TData>({
    ...
    watch(
        () =>
            Object.fromEntries(
                Object.entries(responsiveColumnVisibility).map(([columnId, isVisible]) => [
                    columnId,
                    toValue(isVisible),
                ]),
