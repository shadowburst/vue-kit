...
dataset('filamentResources', fn (): array => discoverPhpClasses(
    dirname(__DIR__, 3).'/app/Filament/Resources',
    'App\\Filament\\Resources',
    '*Resource.php',
));

beforeEach(function (): void {
    actingAs(makeSchemaOperator());
});
...
function makeSchemaOperator(): User
{
...
function makeRecordForResource(string $resourceClass): object
{
    return match ($resourceClass) {
        ...
        default                     => throw new RuntimeException(
            "No record factory is defined for resource [{$resourceClass}].",
        ),
    };
}
...
function assertResourceColumnsAreCovered(
    ...
    expect($missingColumns)->toBeEmpty(
        collect($missingColumns)
            ->map(
                fn (string $column): string => "Column `{$column}` exists on `{$modelClass}` but is not surfaced in `{$resourceClass}` (form, table, or infolist) and is not in the allow-list. Add it to the Resource, or to the allow-list with a comment justifying the exclusion.",
            )
            ->implode("\n"),
    );
}
