...
it('registers the operator dashboard widgets in the admin panel', function (): void {
    $widgets = (new AdminPanelServiceProvider(app()))
        ->panel(Panel::make())
        ->getWidgets();
...
it('derives free tier from teams without an active subscription', function (): void {
    ...
    expect($data['labels'])->toBe(['Free', 'Pro'])->and($data['datasets'][0]['data'])->toBe([3, 1]);
});
...
it('surfaces signup counts for the 7 and 30 day windows', function (): void {
    ...
    expect($data['labels'])->toBe(['Last 7 days', 'Last 30 days'])->and($data['datasets'][0]['data'])->toBe([3, 5]);
});

it('caches each widget query for 60 seconds using the widget class as the key', function (
    string $widgetClass,
    string $method,
): void {
