...
beforeEach(function (): void {
    Route::get('/_test/team', function () {
        return response()->json([
            'currentTeam'   => app()->has('currentTeam') ? app('currentTeam')->id : null,
            'canViewAny'    => Auth::user()?->can(PermissionName::UserViewAny->value),
...
test('user with no team membership is redirected and cannot access the protected resource', function (): void {
    $this->seed(RolePermissionSeeder::class);
    $nonMember = User::factory()->create(['current_team_id' => null]);
